...
const SecondaryButton: React.FC<SecondaryButtonProps> = ({ title, className, icon, onClick }) => {
  return (
    <button
      ...
      onClick={onClick}
    >
      <div className="px-8 py-2 bg-black rounded-[6px] relative group transition duration-200 hover:bg-transparent text-white flex items-center justify-center gap-2">
        {title} {icon}
      </div>
    </button>
  );
